...
class MarkApp:
    ...
    async def _run_setup(self) -> bool:
        ...
        provider = setup_config.get("provider", "gemini")
        model = setup_config.get("model", "gemini-1.5-flash")
    ...
    async def _handle_model_command(self, args: List[str]):
        """Handle /model command - unified provider/model/language selection."""
        ...
        # STEP 3: Language Selection (only for Gemma models)
        if "gemma-3" in selected_model:
            self.console.print()
            self.console.print("[bold cyan]━━━ SELECT LANGUAGE ━━━[/]")
            languages = {
                "en": "English",
                "it": "Italiano", 
                "de": "Deutsch",
            ...
            for i, (code, name) in enumerate(lang_list, 1):
                indicator = "→" if code == self.config.language else " "
                self.console.print(f"  {indicator} [{i}] {name} ({code})")
            
            self.console.print(f"\nChoice (1-{len(languages)}, default 1 for English): ", end="")
            ...
            final_model_name = f"{selected_model}-{lang_code}"
            self.console.print(f"  ✓ Language: {languages[lang_code]}")
    ...
    async def _handle_kleos_mode(self, original_prompt: str):
        """Handle the multi-stage Kleòs mode logic."""
