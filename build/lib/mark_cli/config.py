...
AVAILABLE_MODELS = {
    "gemini": [
        ...
        "gemini-3-flash",
        "gemma-3-27b", # Base Entry, sub-menu for language will handle variants
...
SISTEMA KLEÒS:
- MARK integra il sistema "Kleòs", un tool in grado di ottimizzare i prompt e migliorare drasticamente la qualità delle risposte.
...
KLEÒS-SYSTEM:
- MARK integriert das "Kleòs"-System, ein Werkzeug zur Optimierung von Prompts und zur drastischen Verbesserung della Antwortqualität.
...
SYSTÈME KLEÒS :
- MARK intègre le système "Kleòs", un outil capable d'optimiser le prompt et d'améliorer radicalement la qualità des réponses.
...
UI_MESSAGES = {
    "en": {
        ...
        "kleos_error_analyst": "Error during Kleòs analysis: {e}",
...
def get_api_key(provider: str) -> Optional[str]:
    ...
    try:
        import keyring
        return keyring.get_password("mark", f"{provider}_api_key")
    except Exception:
