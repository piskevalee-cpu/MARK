...
class MarkInterface:
    ...
    def print_welcome(self):
        ...
        banner.append("  ╚═╝     ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝\n", style=f"bold {COLORS['ai']}")
        banner.append("\n")
        banner.append("  Now with Kleòs system! ", style=f"italic {COLORS['muted']}")
    ...
    def print_kleos_user_message(self, content: str, user_name: str = "Utente", timestamp: Optional[str] = None):
        """Print a Kleòs user message with yellow outline."""
        from rich import box
        text = Text()
        text.append(f"{user_name} (Kleòs)", style=f"bold {COLORS['kleos']}")
...
class SetupWizard:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        self.console.print(Text(
            "   [2] Groq - Fast inference (GPT-OSS, Llama, Kimi K2)",
            style=COLORS["info"]
        ))
        ...
        provider = "gemini" if provider_choice != "2" else "groq"
        ...
        if provider == "gemini":
            ...
            key_prompt = "   Enter your Gemini API key: "
        else:
            self.console.print(Text(
        ...
        if provider == "gemini":
            models = [
                ("1", "gemini-2.5-flash", "Latest Flash model"),
                ("2", "gemini-2.5-flash-lite", "Lightweight version"),
                ("3", "gemini-3-flash", "Next Gen Flash"),
                ("4", "gemma-3-27b", "Gemma 3 (Multilingual)"),
            ]
            model_map = {
                "1": "gemini-2.5-flash", 
        ...
        # Language selection for Gemma (Gemini only)
        if selected_model == "gemma-3-27b":
            ...
            lang_choice = Prompt.ask(
                Text("   Code (default: en)", style=COLORS["info"]),
                default="en",
                console=self.console
            ).lower()
            
            valid_codes = [l[0] for l in langs]
            if lang_choice not in valid_codes:
                lang_choice = "en"
            
            config["language"] = lang_choice
            config["model"] = f"{selected_model}-{lang_choice}"
